...
class FrankoClientLibTest extends MyCakeTestCase {

	public $c = NULL;

	public function setUp() {
		$path = Configure::read('Franko.config_path');
		if (!$path) {
			$path = APP . 'Config' . DS;
		}
	...
	if (!isset($this->c))
		$this->c = new FrankoClient("http", "fnordbagger", "spambots", "localhost", 8332, null, 0);
	}
	...
	$junk = new FrankoClient("http", "bobo", "mypass", "kremvax@kremlin.su", 3.14159);
	}
	/**
	 * @expectedException FrankoClientException
	 */
	...
	$junk = new FrankoClient("http", "bobo", "mypass", "kremvax@kremlin.su", 65536);
	}
