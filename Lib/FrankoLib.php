...
/**
 * CakePHP1.3 Wrapper for PHP Franko Library (Vendor)
 * - adds more methods
 * - offline mode available for localhost development (does not through exceptions if not connected to frankod daemon)
 ...
 * 2011-07-19 ms
 */
class FrankoLib extends Franko {
	...
	public function __construct($settings = array()) {
		$this->defaults = array_merge($this->defaults, (array)Configure::read('Franko'));
		...
		if (!$this->C) {
			$this->C = new FrankoClient($scheme, $username, $password, $address, $port, $certificate, $debug);
		}
	}
	...
	public function getBalance($account = null) {
		...
		//throw new FrankoClientException('invalid account given');
		return false;
	}
	...
	/**
	 * get number of total frankos in circulation
	 * 2011-07-16 ms
	 */
	public function getTotalFrankos() {
...
from the amount of FRK sent by the person owning the address
	...
	/**
	 * Returns all transactions sent or received by the period-separated Franko
...
hash: transactions in blocks up to and including this block will not be returned.
	...
	/**
	 * shows the number of blocks in the longest block chain (not including the genesis block). Equivalent to Franko's getblockcount
	 * @return int
	 * 2011-07-16 ms
	 */
...
transactions comprising the balance
	...
	/**
	 * @deprecated
	 * note: probably not neccessary as validation is already implemented in franko.php
