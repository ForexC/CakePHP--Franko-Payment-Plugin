 ...
 *
 * If you find this library useful, your donation of Frankos to address
...
class Franko {
  ...
  /*
   * Franko utility functions by theymos
   * Via http://www.franko.org/smf/index.php?topic=1844.0
  ...
  private function decodeBase58($base58) {
    ...
    for ($i = 0; $i < strlen($base58); $i++) {
      $current = (string) strpos(Franko::$base58chars, $base58[$i]);
  ...
  /**
   * Convert a 160-bit Franko hash to a Franko address
   *
   * @author theymos
  ...
  /**
   * Convert a Franko address to a 160-bit Franko hash
   *
   * @author theymos
   * @param string $addr
   * @return string Franko hash
   * @access public
   */
  ...
  /**
   * Determine if a string is a valid Franko address
   *
   * @author theymos
  ...
  /**
   * Convert the input to its 160-bit Franko hash
   *
   * @param string $data
   * @return string
   * @access private
  ...
  /**
   * Convert a Franko public key to a 160-bit Franko hash
   *
   * @param string $pubkey
   * @return string
   * @access public
...
/**
 * Exception class for FrankoClient
 *
 * @author Mike Gogulski
 * 	http://www.gogulski.com/ http://www.nostate.com/
 */
class FrankoClientException extends ErrorException {
...
/**
 * Franko client class for access to a Franko server via JSON-RPC-HTTP[S]
 *
 * Implements the methods documented at https://www.franko.org/wiki/doku.php?id=api
 *
 * @version 0.3.19
 * @author Mike Gogulski
 * 	http://www.gogulski.com/ http://www.nostate.com/
 */
class FrankoClient extends jsonrpc_client {

  /**
   * Create a jsonrpc_client object to talk to the franko server and return it,
   * or false on failure.
   *
   * @param string $scheme
   * 	"http" or "https"
   * @param string $username
   * 	User name to use in connection the Franko server's JSON-RPC interface
   ...
   * @access public
   * @throws FrankoClientException
   */
  public function __construct($scheme, $username, $password, $address = "localhost", $port = 8332, $certificate_path = '', $debug_level = 0) {
    $scheme = strtolower($scheme);
    if ($scheme != "http" && $scheme != "https")
      throw new FrankoClientException("Scheme must be http or https");
    if (empty($username))
      throw new FrankoClientException("Username must be non-blank");
    if (empty($password))
      throw new FrankoClientException("Password must be non-blank");
    $port = (string) $port;
    if (!$port || empty($port) || !is_numeric($port) || $port < 1 || $port > 65535 || floatval($port) != intval($port))
      throw new FrankoClientException("Port must be an integer and between 1 and 65535");
    if (!empty($certificate_path) && !is_readable($certificate_path))
      throw new FrankoClientException("Certificate file " . $certificate_path . " is not readable");
  ...
  /**
   * Test if the connection to the Franko JSON-RPC server is working
   *
  ...
  /**
   * Convert a Franko server query argument to a jsonrpcval
   *
  ...
  public function query($message) {
    if (!$message || empty($message))
      throw new FrankoClientException("Franko client query requires a message");
    ...
    if ($response->faultCode()) {
      throw new FrankoClientException($response->faultString());
    }
    return php_xmlrpc_decode($response->value());
  }

  /*
   * The following functions implement the Franko RPC API as documented at https://www.franko.org/wiki/doku.php?id=api
   */
  ...
  public function backupwallet($destination) {
    if (!$destination || empty($destination))
      throw new FrankoClientException("backupwallet requires a destination");
   ...
   * @return float Franko balance
   * @throws FrankoClientException
   */
  public function getbalance($account = NULL, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FrankoClientException('getbalance requires a numeric minconf >= 0');
    if ($account == NULL)
  ...
  /**
   * Returns boolean true if server is trying to generate frankos, false otherwise.
   *
  ...
  /**
   * Tell Franko server to generate Frankos or not, and how many processors
   * to use.
   *
  ...
  public function setgenerate($generate = TRUE, $maxproc = -1) {
    if (!is_numeric($maxproc) || $maxproc < -1)
      throw new FrankoClientException('setgenerate: $maxproc must be numeric and >= -1');
  ...
  public function getaccount($address) {
    if (!$address || empty($address))
      throw new FrankoClientException("getaccount requires an address");
  ...
  public function getlabel($address) {
    if (!$address || empty($address))
      throw new FrankoClientException("getlabel requires an address");
  ...
  public function setaccount($address, $account = "") {
    if (!$address || empty($address))
      throw new FrankoClientException("setaccount requires an address");
  ...
  public function setlabel($address, $label = "") {
    if (!$address || empty($address))
      throw new FrankoClientException("setlabel requires an address");
    return $this->query("setlabel", $address, $label);
  }

  /**
   * Returns a new franko address for receiving payments.
   *
   ...
   * @return float Franko total
   * @throws FrankoClientException
   */
  public function getreceivedbyaddress($address, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FrankoClientException('getreceivedbyaddress requires a numeric minconf >= 0');
    if (!$address || empty($address))
      throw new FrankoClientException("getreceivedbyaddress requires an address");
  ...
  public function getreceivedbyaccount($account, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FrankoClientException('getreceivedbyaccount requires a numeric minconf >= 0');
    if (!$account || empty($account))
      throw new FrankoClientException("getreceivedbyaccount requires an account");
  ...
  public function getreceivedbylabel($label, $minconf = 1) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FrankoClientException('getreceivedbylabel requires a numeric minconf >= 0');
    if (!$label || empty($label))
      throw new FrankoClientException("getreceivedbylabel requires a label");
  ...
  /**
   * Return an array of arrays showing how many Frankos have been received by
  ...
  public function listreceivedbyaddress($minconf = 1, $includeempty = FALSE) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FrankoClientException('listreceivedbyaddress requires a numeric minconf >= 0');
  ...
  public function listreceivedbyaccount($minconf = 1, $includeempty = FALSE) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FrankoClientException('listreceivedbyaccount requires a numeric minconf >= 0');
  ...
  public function listreceivedbylabel($minconf = 1, $includeempty = FALSE) {
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FrankoClientException('listreceivedbylabel requires a numeric minconf >= 0');
   ...
   *
   * @param string $address Destination Franko address or IP address
  ...
  public function sendtoaddress($address, $amount, $comment = NULL, $comment_to = NULL) {
    if (!$address || empty($address))
      throw new FrankoClientException("sendtoaddress requires a destination address");
    if (!$amount || empty($amount))
      throw new FrankoClientException("sendtoaddress requires an amount to send");
    if (!is_numeric($amount) || $amount <= 0)
      throw new FrankoClientException("sendtoaddress requires the amount sent to be a number > 0");
  ...
  /**
   * Check that $address looks like a proper Franko address.
   *
   * @param string $address String to test for validity as a Franko address
   * @return array An array containing:
   * 	"isvalid" => true or false
   * 	"ismine" => true if the address is in the server's wallet
   * 	"address" => frankoaddress
   *  Note: ismine and address are only returned if the address is valid.
   * @throws FrankoClientException
   */
  public function validateaddress($address) {
    if (!$address || empty($address))
      throw new FrankoClientException("validateaddress requires a Franko address");
  ...
  public function gettransaction($txid) {
    if (!$txid || empty($txid) || strlen($txid) != 64 || !preg_match('/^[0-9a-fA-F]+$/', $txid))
      throw new FrankoClientException("gettransaction requires a valid hexadecimal transaction ID");
    return $this->query("gettransaction", $txid);
  }

  /**
   * Move frankos between accounts.
   *
   * @param string $fromaccount
   *    Account to move from. If given as an empty string ("") or NULL, frankos will
   ...
   *     Amount to move
   * @param integer $minconf
   *     Minimum number of confirmations on frankos being moved
   * @param string $comment
   *     Transaction comment
   * @throws FrankoClientException
   * @since 0.3.18
  ...
  public function move($fromaccount = "", $toaccount, $amount, $minconf = 1, $comment = NULL) {
    ...
    if (!$toaccount || empty($toaccount) || !$amount || !is_numeric($amount) || $amount <= 0)
      throw new FrankoClientException("move requires a from account, to account and numeric amount > 0");
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FrankoClientException('move requires a numeric $minconf >= 0');
   ...
   * if there is less than $amount frankos with $minconf confirmations in the
   ...
   * @param string $toaddress Franko address to send to
   * @param float $amount Amount to send
   * @param integer $minconf Minimum number of confirmations on frankos being sent
   * @param string $comment
   * @param string $comment_to
  ...
  public function sendfrom($account, $toaddress, $amount, $minconf = 1, $comment = NULL, $comment_to = NULL) {
    if (!$account || !$toaddress || empty($toaddress) || !$amount || !is_numeric($amount) || $amount <= 0)
      throw new FrankoClientException("sendfrom requires a from account, to account and numeric amount > 0");
    if (!is_numeric($minconf) || $minconf < 0)
      throw new FrankoClientException('sendfrom requires a numeric $minconf >= 0');
    if (!$comment && !$comment_to)
  ...
  /**
   * Return the current franko address for receiving payments to $account.
   * The account and address will be created if $account doesn't exist.
   *
   * @param string $account Account name
   * @return string Franko address for $account
   * @throws FrankoClientException
   * @since 0.3.18
   */
  public function getaccountaddress($account) {
    if (!$account || empty($account))
      throw new FrankoClientException("getaccountaddress requires an account");
   ...
   * @param string $account
   * @return array
   *    A simple array of Franko addresses associated with $account, empty
   *    if the account doesn't exist.
   * @throws FrankoClientException
   */
  public function getaddressesbyaccount($account) {
    if (!$account || empty($account))
      throw new FrankoClientException("getaddressesbyaccount requires an account");
